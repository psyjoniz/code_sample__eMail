...
class eMail {
	...
	function send() {
		...
		switch($this->trans_type) {
			...
			case 'socket' : default :
				...
				if(!$this->smtp_sock) {
					die('SMTP Socket failed: ' . $errno . ' - ' . $errstr);
					$this->cleanUp();
					return false;
				...
				if(true !== $this->getResponseFromInputToSocket('data', '354')) {
					die('eMail->getResponseFromInputToSocket() failed');
					$this->cleanUp();
					return false;
				} else { if($this->debug) { echo('Got the response we were looking for<br />'); } }
	...
	function getInitialResponseFromSocket($expectedresponse) {
		...
		if($response == $expectedresponse) {
			...
			die('Initial SMTP response unexpected: ' . $response . ' (trailing response: ' . $trailing_response . ')');
			return false;
		}
		if($this->debug) {
